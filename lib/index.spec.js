...
describe('lib/index --- ', () => {
    ...
    it('should check for --all', () => {
        ...
        expect(childProcessSpy.getCall(7).args[1]).to.be.eql({
            cwd: './node_modules/.bin/'
        });
        expect(childProcessSpy.getCall(8).args[0]).to.be.eql('tslint --version');
        expect(childProcessSpy.getCall(8).args[1]).to.be.eql({
            cwd: './node_modules/.bin/'
        });
        expect(childProcessSpy.getCall(9).args[0]).to.be.eql('nvm --version');
        expect(childProcessSpy.getCall(9).args[1]).to.be.eql({
            cwd: './node_modules/.bin/'
        });
        expect(childProcessSpy.getCall(10).args[0]).to.be.eql('n --version');
        expect(childProcessSpy.getCall(10).args[1]).to.be.eql({
            cwd: './node_modules/.bin/'
        });
    });
