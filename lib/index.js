...
function VersionManager() {
    this.options = ['node', 'npm', 'yarn', 'git', 'karma', 'gulp', 'grunt', 'eslint', 'tslint', 'nvm', 'n'];
}
...
function findOutVersion(key) {
    return String(require('child_process').execSync(key + ' --version', { cwd: './node_modules/.bin/' })).replace(
        /[^0-9.]/gm,
        ''
    );
}
